...
app = FastAPI(
    title="iBanking Authentication API",
    description="TDTU Tuition Payment System - Authentication Service",
    version="1.0.0"
)

# ============================================================
# CRITICAL: CORS MIDDLEWARE MUST BE HERE
# ============================================================
app.add_middleware(
    CORSMiddleware,
...
async def root():
    ...
    return {
        "message": "Welcome to iBanking Authentication API",
        "status": "running",
        "version": "1.0.0",
        "docs": "/docs"
    }
...
async def login_for_access_token(form_data : Annotated[OAuth2PasswordRequestForm, Depends()]):
    ...
    if not user or not verify_password(form_data.password, user.password_hash):
        print(f"Login failed for user: {form_data.username}")  # Debug log
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
    ...
    print(f"Login successful for user: {form_data.username}")  # Debug log
    
    return {'access_token' : access_token, 'token_type' : 'bearer'}

@app.get('/customers/me', response_model=Customer, tags=['Authentication'])
